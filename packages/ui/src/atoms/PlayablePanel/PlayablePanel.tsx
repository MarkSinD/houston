...
import plusBetDisabled from '@monorepo-example/resource/images/playable-panel/plus-bet-disabled.png'
import plusBet from '@monorepo-example/resource/images/playable-panel/plus-bet.png'
import startButton from '@monorepo-example/resource/images/playable-panel/start-button.png'
import stopButton from '@monorepo-example/resource/images/playable-panel/stop-button.png'
...
const PlayablePanel: FC<PlayablePanelProps> = ({
	...
	const getBetTiles = (selectedAutoBet: number, isRight: boolean) =>
		autoBets.map((autoBet, index) => (
			<HoverComponent
				key={index}
				autoBet={String(autoBet)}
				className={`${isStartClicked ? classes.autoBetTileDisabled : ''} ${classes.autoBetTile} ${
	...
	return (
		<>
			{isSingleBetMode ? (
				<div
					className={`${classes.playablePanelContainer} ${
				...
				>
					...
					<div className={classes.betPanel}>
						<div className={classes.autoBetTiles}>{getBetTiles(autoBetLeft, false)}</div>
						<div className={classes.betPanelContainer}>
							<BetPanel autoBetValue={autoBetLeft} isSingleBetMode={isSingleBetMode} />
						</div>
					</div>
					<img
						alt=""
				...
				>
					...
					<div className={classes.betPanel}>
						<div className={classes.autoBetTilesNonSingleMode}>
							<div className={classes.autoBetTiles}>{getBetTiles(autoBetLeft, false)}</div>
							<div className={classes.autoBetTiles}>{getBetTiles(autoBetRight, true)}</div>
						</div>
						<div
							className={`${classes.betPanelContainer} ${
								isSingleBetMode ? '' : classes.nonSingleMode
							}`}
						>
							<BetPanel autoBetValue={autoBetLeft} isSingleBetMode={isSingleBetMode} />
							<BetPanel autoBetValue={autoBetRight} isSingleBetMode={isSingleBetMode} />
						</div>
					</div>
					<img
						alt=""
