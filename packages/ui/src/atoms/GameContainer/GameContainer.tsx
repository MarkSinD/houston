...
const GameContainer: FC<GameContainerProps> = ({ handleBackgroundAnimation = () => {} }) => {
	...
	useEffect(() => {
		...
		const durationSeconds = rand < 50 ? 1 + Math.random() : 1 + Math.random() * 100
	...
	return (
		<>
			...
			<div className={classes.gameCoefficientScale}>
				<BetCoefficientScale coefficient={flightDuration} isAction={isRocketLaunched} />
			</div>
		</>
	)
}
